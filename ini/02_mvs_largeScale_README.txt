...
doRemoveHugeTriangles=FALSE				: This option causes that the appliaction will remove huge triangles.

[delanuaycut]
saveMeshTextured=FALSE					: Set this to FALSE if do not want to have textured mesh of (each part) on the output. You can texture it later using mvs_retexture.ini. The colored mesh will be computed anyway!

[largeScale]
workDirName="largeScaleMaxPts01024"
doReconstructSpaceAccordingToVoxelsArray=TRUE
doGenerateAndReconstructSpaceMaxPts=FALSE
...
planMaxPtsPerVoxel=3000000				: Maximim number of vertices in the tetrahedralization. Set it to smaller value if you have less than 12GB of ram.
nGridHelperVolumePointsDim=10
joinMeshesSaveTextured=FALSE				: Set it to FALSE in order to turn off the computation of textured joined mesh of all parts.

[meshEnergyOpt]
doOptimizeOrSmoothMesh=FALSE				: Set it to TRUE if you want to run final mesh refinement step on each part. Note that this is experimental feature. We are still working on it. It is suitable for beta testing. The dafault vaule is FALSE. Let us know your opinion on the result when using TRUE please.
